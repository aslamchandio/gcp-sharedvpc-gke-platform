...
locals {
  ...
  # Cluster name = prefix + suffix.  e.g. it-prod-standard
  ...
  network_name      = "${local.name}-vpc"   # it-prod-vpc (-> -router, -nat, -*-firewall)
  proxy_subnet_name = "${local.name}-proxy" # it-prod-proxy

  # Subnet names in tfvars are bare suffixes; prefix them here. e.g. it-prod-gke
  prefixed_subnets = [
